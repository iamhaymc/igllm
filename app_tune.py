...
The engine reads a QAT-packed checkpoint and has no trainer of its own, so a
tune happens on the reference side: TRL's `SFTTrainer` trains a LoRA adapter
over the frozen base weights, and the adapter is then folded back into the
float checkpoint so the ordinary packing path can take it. The tune is a
...
def need_modules():
    ...
    try:
        ...
        print("skip: %s; run `python3 run.py install`" % miss)
        return None
    return True
...
def train_step(model_path, out_path, data_path, epochs, batch, rate, cut):
    ...
    adapter = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        # The towers stay frozen: the example tune is a text tune, and a tower
        # adapter would not survive the packing path this script folds into.
...
def main():
    parser = argparse.ArgumentParser(description="igllm fine tuning with TRL")
